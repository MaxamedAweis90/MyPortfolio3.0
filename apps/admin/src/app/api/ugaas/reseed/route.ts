...
import { Project } from "@portfolio/database";
import { Experience } from "@portfolio/database";
import { Certificate } from "@portfolio/database";
  ...
  try {
    ...
    const formattedProjects = projectsData.map((p, index) => ({
      ...
      image:
        ...
        p.screenshots?.[0] ||
        "/Hero3DMe.png",
      tools: p.tools?.map((t: any) => t.title || "").filter(Boolean) || [],
    ...
    try {
      await Promise.all([
        triggerRevalidate("portfolio-projects"),
        triggerRevalidate("portfolio-experience"),
        triggerRevalidate("portfolio-settings"),
        triggerRevalidate("portfolio-certificates"),
      ]);
    } catch (revErr) {
      console.warn("Reseed revalidation warning:", revErr);
    }
