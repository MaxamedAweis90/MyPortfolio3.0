...
function normalizeExperience(doc: any): ExperienceItem {
  if (doc.period && doc.technologies) return doc as ExperienceItem;
  return {
    id: doc._id?.toString() || doc.id || `exp-${Math.random()}`,
    role: doc.role || "Software Engineer",
    company: doc.company || "Company",
    companyShort: doc.company ? doc.company.slice(0, 4).toUpperCase() : "EXP",
    location: doc.location || doc.badges?.[0] || "Banadir, Somalia",
    period: doc.period || doc.duration || "Present",
    type: doc.type || doc.badges?.[1] || "Career Experience",
    badgeBg: doc.badgeBg || "bg-blue-500/10",
    badgeColor: doc.badgeColor || "text-blue-400 border-blue-500/30",
    highlights: doc.highlights || [],
    technologies: doc.technologies || doc.techStack || [],
  };
}

export default function ExperienceSection({ initialExperiences, experiences: propExperiences }: ExperienceSectionProps = {}) {
  const rawExperiences = propExperiences || initialExperiences || [];
  const experiences = rawExperiences.map(normalizeExperience);
