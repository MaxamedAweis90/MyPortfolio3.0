...
import { mapMongoProjectToPortfolio } from "@/lib/portfolio-mappers";
...
export default function MyWorkSection({ initialProjects, projects: propProjects }: MyWorkSectionProps = {}) {
  // Only render projects where isFeatured: true (capped at max 6 items)
  const featuredProjects = React.useMemo(() => {
    const raw = (propProjects || initialProjects || []).map((p) =>
      p.shortTagline ? (p as Project) : mapMongoProjectToPortfolio(p)
    );
    const featured = raw.filter((p) => p.isFeatured);
    return (featured.length > 0 ? featured : raw).slice(0, 6);
  }, [initialProjects, propProjects]);
