...
export const animatedPageIn = () => {
  ...
  const front = [1,2,3,4].map(n => document.getElementById(`banner-${n}`)).filter(Boolean);
  const back  = [1,2,3,4].map(n => document.getElementById(`banner-back-${n}`)).filter(Boolean);
...
export const animatedPageOut = (href, router) => {
  const front = [1,2,3,4].map(n => document.getElementById(`banner-${n}`)).filter(Boolean);
  const back  = [1,2,3,4].map(n => document.getElementById(`banner-back-${n}`)).filter(Boolean);
